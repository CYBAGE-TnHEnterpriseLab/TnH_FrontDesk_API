...
## Quick start
```powershell
# from repository root, install shared local module once
mvn -f .\pms-security-common\pom.xml clean install

# run backend-frontdesk (skip test compilation during local run)
mvn -f .\backend-frontdesk\pom.xml spring-boot:run -Dmaven.test.skip=true
```
...
```bash
curl -X POST http://localhost:8082/api/uploads/images \
