...
public class PropertyServiceImpl implements PropertyService {
    ...
    public PropertyResponseDTO createDraft(PropertyRequestDTO request) {
        ...
        validateUniqueness(trim(request.getName()), trim(request.getEmail()), null);
    ...
    public PropertyResponseDTO updateProperty(UUID propertyId, PropertyRequestDTO request) {
        ...
        String requestedName = request.getName() != null ? trim(request.getName()) : property.getName();
        String requestedEmail = request.getEmail() != null ? trim(request.getEmail()) : property.getEmail();
        validateUniqueness(requestedName, requestedEmail, propertyId);
    ...
    private void validateUniqueness(String name, String email, UUID currentPropertyId) {
        if (name != null) {
            boolean nameExists = repository.existsByNameIgnoreCase(name);
            if (nameExists && !isSamePropertyByName(currentPropertyId, name)) {
        ...
        if (email != null) {
            ...
            if (emailExists && !isSamePropertyByEmail(currentPropertyId, email)) {
                throw new BadRequestException("Property email already exists");
            }
        }
    }

    private boolean isSamePropertyByName(UUID currentPropertyId, String name) {
        if (currentPropertyId == null) {
            return false;
        }
        return repository.findById(currentPropertyId)
                .map(property -> property.getName() != null && property.getName().equalsIgnoreCase(name))
                .orElse(false);
    }

    private boolean isSamePropertyByEmail(UUID currentPropertyId, String email) {
        if (currentPropertyId == null) {
            return false;
        }
        return repository.findById(currentPropertyId)
                .map(property -> property.getEmail() != null && property.getEmail().equalsIgnoreCase(email))
                .orElse(false);
    }
